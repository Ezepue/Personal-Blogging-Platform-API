...
def update_existing_article(id: int, article: ArticleCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def delete_existing_article(id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    if delete_article(db, id, user.id):  # Pass user.id
